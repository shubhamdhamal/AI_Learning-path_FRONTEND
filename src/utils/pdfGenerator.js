...
const generateLearningPathHTML = (pathData) => {
      ...
      <div class="milestone">
        <div class="milestone-header">
          ...
          <div class="milestone-info">
            ...
            <span class="milestone-duration">⏱ ${milestone.duration_weeks || 1} week${milestone.duration_weeks !== 1 ? 's' : ''}</span>
          </div>
        </div>
  ...
  // Generate job market insights HTML
  const jobMarketHTML = jobMarket ? `
    <div class="section job-market">
      <h2>💼 Job Market Insights</h2>
      <div class="insights-grid">
        ${jobMarket.average_salary ? `
          <div class="insight-item">
            <span class="insight-label">Average Salary</span>
            <span class="insight-value">${jobMarket.average_salary}</span>
          </div>
        ` : ''}
        ${jobMarket.demand ? `
          <div class="insight-item">
            <span class="insight-label">Market Demand</span>
            <span class="insight-value">${jobMarket.demand}</span>
          </div>
        ` : ''}
        ${jobMarket.growth ? `
          <div class="insight-item">
            <span class="insight-label">Growth Rate</span>
            <span class="insight-value">${jobMarket.growth}</span>
          </div>
        ` : ''}
      </div>
      ${jobMarket.related_roles && jobMarket.related_roles.length > 0 ? `
        <div class="related-roles">
          <h4>👥 Related Roles</h4>
          <p>${jobMarket.related_roles.join(', ')}</p>
        </div>
      ` : ''}
    </div>
  ` : '';
